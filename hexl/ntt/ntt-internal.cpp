...
NTT::NTT(size_t degree, uint128_t q, uint128_t root_of_unity,
         std::shared_ptr<AllocatorBase> alloc_ptr)
    : m_degree(degree),
      m_q(q),
      m_w(root_of_unity),
      m_alloc(alloc_ptr),
      m_aligned_alloc(AlignedAllocator<uint128_t, 64>(m_alloc)),
      m_root_of_unity_powers(m_aligned_alloc),
      // m_precon32_root_of_unity_powers(m_aligned_alloc),
      m_precon128_root_of_unity_powers(m_aligned_alloc),
      ...
      m_precon128_inv_root_of_unity_powers(m_aligned_alloc),
  ...
  m_degree_bits = static_cast<size_t>(std::log2l(m_degree));
  m_w_inv = InverseMod(m_w, m_q);
  ComputeRootOfUnityPowers();
}

NTT::NTT(size_t degree, uint128_t q, std::shared_ptr<AllocatorBase> alloc_ptr)
    : NTT(degree, q, MinimalPrimitiveRoot(2 * degree, q), alloc_ptr) {}

void NTT::ComputeRootOfUnityPowers() {
    AlignedVector64<uint128_t> root_of_unity_powers(m_degree, 0, m_aligned_alloc);
  AlignedVector64<uint128_t> inv_root_of_unity_powers(m_degree, 0,
                                                     m_aligned_alloc);
  ...
  // roots of unity
  // AlignedVector64<uint128_t> W2_roots;
  ...
  // By creating these duplicates, we avoid extra permutations while loading the
  // roots of unity
  // AlignedVector64<uint128_t> W4_roots;
  ...
  auto compute_barrett_vector = [&](const AlignedVector64<uint128_t>& values,
                                    uint64_t bit_shift) {
    AlignedVector64<uint128_t> barrett_vector(m_aligned_alloc);
    for (uint128_t value : values) {
  ...
  m_precon128_root_of_unity_powers =
      compute_barrett_vector(root_of_unity_powers, 128);
  ...
  AlignedVector64<uint128_t> temp(m_degree, 0, m_aligned_alloc);
  ...
  m_precon128_inv_root_of_unity_powers =
      compute_barrett_vector(m_inv_root_of_unity_powers, 128);
}

bool NTT::CheckArguments(uint128_t degree, uint128_t modulus) {
  HEXL_UNUSED(degree);
  HEXL_UNUSED(modulus);
  HEXL_CHECK(IsPowerOfTwo(degree),
             "degree " << degree << " is not a power of 2");
  HEXL_CHECK(degree <= (uint128_t{1ULL} << NTT::MaxDegreeBits()),
             "degree should be less than 2^" << NTT::MaxDegreeBits() << " got "
                                             << degree);
  HEXL_CHECK(modulus <= (uint128_t{1ULL} << NTT::MaxModulusBits()),
...
void NTT::ComputeForward(uint128_t* result, const uint128_t* operand,
                         uint128_t input_mod_factor,
...
//     const uint128_t* root_of_unity_powers = GetAVX512RootOfUnityPowers().data();
//     const uint128_t* precon_root_of_unity_powers =
...
// #endif
  ...
  const uint128_t* root_of_unity_powers = GetRootOfUnityPowers().data();
  const uint128_t* precon_root_of_unity_powers =
      GetPrecon128RootOfUnityPowers().data();
...
void NTT::ComputeInverse(uint128_t* result, const uint128_t* operand,
                         uint128_t input_mod_factor,
                         uint128_t output_mod_factor) {
  HEXL_CHECK(result != nullptr, "result == nullptr");
  HEXL_CHECK(operand != nullptr, "operand == nullptr");
...
//     const uint128_t* inv_root_of_unity_powers = GetInvRootOfUnityPowers().data();
...
// #endif
  ...
  const uint128_t* inv_root_of_unity_powers = GetInvRootOfUnityPowers().data();
  const uint128_t* precon_inv_root_of_unity_powers =
      GetPrecon128InvRootOfUnityPowers().data();
